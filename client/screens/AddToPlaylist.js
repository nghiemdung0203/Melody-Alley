...
const AddToPlaylist = ({ route }) => {
    ...
    const getRecentlyPlaylist = async () => {
        if (user.id !== undefined) {
            await axios.get(`http://10.0.2.2:5002/Playlist/GetPlaylist?UserID=${user.id}`).then((response) => {
    ...
    const renderItem = ({ item, index }) => {
        return (
            <View style={styles.songContainer}>
                <TouchableOpacity style={styles.songInfo} onPress={() => AddSongToPlaylist(item._id)}>
    ...
    return (
        <View style={{
            ...
            <BottomSheet bottomSheetRef={refRBSheet}>
                <CreatePlaylistBottomSheet onClose={closeRBSheet} user={user} song={song} onPlaylistCreated={handlePlaylistCreated} />
            </BottomSheet>

            {/**body */}
            <View style={styles.bodyContainer}>
                <Text style={styles.bodyTextPlaylist}>
                    Recently added
                </Text>
                <FlatList
                    data={userPlaylist}
